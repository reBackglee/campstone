  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-3">
            <span className="text-3xl">📂</span> Kategoriler
          </h1>
          <p className="text-slate-400 mt-1">Kitap kategorilerini yönetin</p>
        </div>
        <button
          onClick={() => {
            resetForm()
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-slate-700/30">
              ...
              <th className="text-left text-slate-300 font-medium px-6 py-4">Kategori Adı</th>
              <th className="text-left text-slate-300 font-medium px-6 py-4">Açıklama</th>
          ...
          <tbody>
            {categories.map((category, index) => (
              ...
              >
                <td className="px-6 py-4 text-slate-400">{category.id}</td>
      ...
      <ConfirmDialog
        ...
        message={`"${selectedCategory?.name}" kategorisini silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`}
      />
    </div>
  )
}
